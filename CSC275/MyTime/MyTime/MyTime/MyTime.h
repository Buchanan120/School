...
private:
	...
	// the following static fields might come in handy
		// 8,639,999 _ticks = 23:59:59.99 i.e. 11:59:59.99pm
...
// CONSTRUCTORS
	...
	MyTime(int initHours, int initMinutes, int initSeconds, int initMilisecs);
...
// METHODS
	...
	// SetTime (various useful overloads)
		// Hours, Minutes, Seconds, Miliseconds
	void SetTime(int newHours, int newMinutes, int newSeconds, int newMilisecs);
		// Hours, Minutes, Seconds
	...
	void SubtractTicks(long lessTicks);
		// MyTime
	void SubtractTime(const MyTime & aMyTime);
