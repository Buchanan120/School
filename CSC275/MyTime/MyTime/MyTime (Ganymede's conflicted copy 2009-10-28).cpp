...
// METHODS
	...
	static MyTime MyTime::Now() {
		...
		long theseTicks = theseTicks % _ticksPerDay;
		int currentTime = 0;
		currentTime += (timeInfo->tm_hour * _ticksPerHour) 
			+= (timeInfo->tm_min * _ticksPerMinute) 
			+= (timeInfo->tm_sec * _ticksPerSecond);
		MyTime aMyTime(currentTime);
		return aMyTime;	
	}
	...
	MyTime MyTime::operator= (const MyTime & aMyTime) {
		aMyTime.SetTicks(aMyTime._ticks);
		return *this;
	}
	...
	friend ostream & operator<< (ostream & os, const MyTime & aMyTime) {
		// insert formatted time into stream
		os	<< aMyTime.GetHours() << ":" 
			<< aMyTime.GetMinutes() << ":" 
			<< aMyTime.GetSeconds() << "."
			<< (aMyTime.GetTicks() % _ticksPerSecond);
