...
// UTILITY FUNCTIONS
	...
	int GetLeapYears(MyDate & aMyDate);

	// return number of days in month
	int DaysInThisMonth();
	int DaysInThisMonth(int month);
	int DaysInThisMonth(MyDate & aMyDate);

	// Return number of days from epoch to beginning of current year
	int NumDaysToYear(int year);

	// return days from start of year to start of month
	static const int numDaysToMonth[14];
